...
class Plotter():
    ...
    def __plot_front(self, front, generation_number):
        ...
        computed_f1 = map(lambda individual: individual.objective_scores[0], front)
        computed_f2 = map(lambda individual: individual.objective_scores[1], front)
        axes.plot(computed_f1, computed_f2, 'g.', label='Computed Pareto Front')

        axes.set_xlabel('Slots')
